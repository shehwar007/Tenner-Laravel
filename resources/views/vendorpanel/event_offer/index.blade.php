  ...
  <div class="row">
    <div class="col-md-12">
      <div class="col-lg-4 offset-lg-1 mt-2 mt-lg-0">
          ...
          <a class="btn btn-secondary btn-sm float-right" href="{{ route('vendor.event_management.edit_event')}}">
            {{ __('Edit Offer') }}
          </a>
          
        <button class="btn btn-danger btn-sm float-right mr-2 d-none bulk-delete"
          data-href="{{ route('vendor.event_management.bulk_delete_event') }}">
                        ...
                        <td>{{ $event->offer_type}}</td>
                         <td>
                            <form id="statusForm-{{ $event->id }}" class="d-inline-block"
                              action="{{ route('vendor.event_management.event.event_status', ['id' => $event->id, 'language' => request()->input('language')]) }}"
                              method="post">
                         ...
                         <td class="text-center">
                            <div>
                              <!--<a href="{{ route('vendor.event_management.edit_event', ['id' => $event->id]) }}"-->
                              <a href="{{ route('vendor.event_management.edit_event') }}"
                              class="btn btn-primary">
                              {{ __('Edit') }}
                              </a>
                      
                          <form class="deleteForm"
                              action={{ route('vendor.event_management.delete_event', ['id' => $event->id]) }}
